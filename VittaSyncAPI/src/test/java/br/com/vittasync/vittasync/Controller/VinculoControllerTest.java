package br.com.vittasync.vittasync.Controller;

import br.com.vittasync.vittasync.DTO.*;
import br.com.vittasync.vittasync.Model.Usuario;
import br.com.vittasync.vittasync.Model.Vinculo;
import br.com.vittasync.vittasync.Repository.VinculoRepository;
import br.com.vittasync.vittasync.Service.*;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.autoconfigure.web.servlet.WebMvcTest;
import org.springframework.boot.test.autoconfigure.web.servlet.AutoConfigureMockMvc;
import org.springframework.boot.test.mock.mockito.MockBean;
import org.springframework.http.MediaType;
import org.springframework.test.web.servlet.MockMvc;

import java.util.List;
import java.util.Optional;

import static org.mockito.ArgumentMatchers.any;
import static org.mockito.ArgumentMatchers.anyInt;
import static org.mockito.Mockito.*;
import static org.springframework.test.web.servlet.request.MockMvcRequestBuilders.*;
import static org.springframework.test.web.servlet.result.MockMvcResultMatchers.*;

@WebMvcTest(VinculoController.class)
@AutoConfigureMockMvc(addFilters = false)
class VinculoControllerTest {

    @Autowired private MockMvc mockMvc;

    @MockBean private VinculoService service;
    @MockBean private JwtService jwtService;
    @MockBean private UsuarioService usuarioService;
    @MockBean private PermissaoService permissaoService;
    @MockBean private VinculoRepository vinculoRepository;
    @MockBean private SessaoService sessaoService;


    @Test
    void testGerarCodigoOk() throws Exception {
        Usuario usuario = new Usuario(); usuario.setId(1); usuario.setCpf("123");
        ConviteVinculoOutputDTO output = new ConviteVinculoOutputDTO(); output.setCodigo("ABC123");

        when(jwtService.extrairCpf("token")).thenReturn("123");
        when(usuarioService.searchByCpf("123")).thenReturn(usuario);
        when(service.gerarCodigo(1)).thenReturn(output);

        mockMvc.perform(post("/vinculos/gerar")
                        .header("Authorization","Bearer token"))
                .andExpect(status().isOk());
    }

    @Test
    void testEntrarComCodigoOk() throws Exception {
        Usuario usuario = new Usuario(); usuario.setId(1); usuario.setCpf("123");
        PacienteResumoDTO paciente = new PacienteResumoDTO(); paciente.setCpf("222");

        when(jwtService.extrairCpf("token")).thenReturn("123");
        when(usuarioService.searchByCpf("123")).thenReturn(usuario);
        when(service.entrarComCodigo(any(), any(), anyInt())).thenReturn(paciente);

        mockMvc.perform(post("/vinculos/entrar")
                        .header("Authorization","Bearer token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"codigo\":\"ABC123\",\"funcao\":\"MEDICO\"}"))
                .andExpect(status().isOk());
    }

    @Test
    void testEnviarEmailOk() throws Exception {
        doNothing().when(service).enviarConviteEmail(any(), any());

        mockMvc.perform(post("/vinculos/enviar-email")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"teste@teste.com\",\"codigo\":\"XYZ789\"}"))
                .andExpect(status().isOk());
    }

    @Test
    void testRemoverVinculoOk() throws Exception {
        Usuario usuario = new Usuario(); usuario.setId(1); usuario.setCpf("123");
        Vinculo vinculo = new Vinculo(); vinculo.setPacienteId(2); vinculo.setUsuarioId(1);

        when(jwtService.extrairCpf("token")).thenReturn("123");
        when(usuarioService.searchByCpf("123")).thenReturn(usuario);
        when(vinculoRepository.findById(1L)).thenReturn(Optional.of(vinculo));
        when(permissaoService.podeRemoverVinculo(1, vinculo)).thenReturn(true);
        doNothing().when(service).removerVinculo(1L, 1);

        mockMvc.perform(delete("/vinculos/1")
                        .header("Authorization","Bearer token"))
                .andExpect(status().isNoContent());
    }

    @Test
    void testListarOk() throws Exception {
        Usuario usuario = new Usuario(); usuario.setId(1); usuario.setCpf("123");
        VinculoOutputDTO dto = new VinculoOutputDTO(); dto.setId(1L);

        when(jwtService.extrairCpf("token")).thenReturn("123");
        when(usuarioService.searchByCpf("123")).thenReturn(usuario);
        when(service.listar(1)).thenReturn(List.of(dto));

        mockMvc.perform(get("/vinculos")
                        .header("Authorization","Bearer token"))
                .andExpect(status().isOk());
    }

    @Test
    void testListarPacientesOk() throws Exception {
        Usuario usuario = new Usuario(); usuario.setId(1); usuario.setCpf("123");
        PacienteResumoDTO paciente = new PacienteResumoDTO(); paciente.setCpf("222");

        when(jwtService.extrairCpf("token")).thenReturn("123");
        when(usuarioService.searchByCpf("123")).thenReturn(usuario);
        when(service.listarPacientesDoUsuario(1)).thenReturn(List.of(paciente));

        mockMvc.perform(get("/vinculos/pacientes")
                        .header("Authorization","Bearer token"))
                .andExpect(status().isOk());
    }

    @Test
    void testListarPorPacienteOk() throws Exception {
        VinculoOutputDTO dto = new VinculoOutputDTO(); dto.setId(1L);

        when(service.listarPorPaciente(2)).thenReturn(List.of(dto));

        mockMvc.perform(get("/vinculos/paciente/2"))
                .andExpect(status().isOk());
    }
}
